    ...
    <>
      ...
      <div className="dsk-exec">
        <div style={{ fontFamily: T.mono, maxWidth: 1100 }}>
          ...
          {/* ══ BLOQUE 5 — DECISIONES AGENTIK ════════════════════════════════════
               "REQUIERE DECISIÓN HOY" — Top 3-4 acciones con impacto financiero real.
               Si no se atiende hoy, genera problema mañana.
               ════════════════════════════════════════════════════════════════ */}
          <SectionHeader
            label="Decisiones Agentik"
            accent={C.blueDark}
            sublabel={`Copilot ejecutivo · ${viewCtx.actionFocus}`}
          />

          {/* Decisiones urgentes — panel ejecutivo */}
          <div style={{
            background: "var(--ag-grad-ai, linear-gradient(135deg, #001E4A 0%, #003A8A 100%))",
            borderRadius: R.card, padding: `${S[4]}px ${S[5]}px`, marginBottom: S[4],
          }}>
            <div style={{
              display: "flex", alignItems: "center", gap: S[3], marginBottom: S[3],
            }}>
              <div style={{
                fontFamily: T.mono, fontSize: T.sz.sm, fontWeight: T.wt.black,
                color: "#ffffff", textTransform: "uppercase" as const,
                letterSpacing: "0.06em",
              }}>
                Requiere decisión hoy
              </div>
              {(criticalAlerts.length > 0 || (cobrosBreakdown?.consignacionesPendientes.count ?? 0) > 0 || openTasks > 0) && (
                <div style={{
                  fontFamily: T.mono, fontSize: T.sz["2xs"], fontWeight: T.wt.bold,
                  ...
                  Si no atiendes esto hoy, tendra consecuencias
                </div>
              )}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column" as const, gap: S[2] }}>
              {[
                ...(criticalAlerts.length > 0 ? [{
                  label: `Revisar ${criticalAlerts.length} alerta${criticalAlerts.length > 1 ? "s" : ""} crítica${criticalAlerts.length > 1 ? "s" : ""}`,
                  context: "Requieren decisión ejecutiva inmediata",
                  href: `/${orgSlug}/alerts`,
                  badgeColor: "#f87171",
                  badge: "CRÍTICO",
                }] : []),
                ...(cobrosBreakdown && cobrosBreakdown.consignacionesPendientes.count > 0 ? [{
                  label: `Identificar ${cobrosBreakdown.consignacionesPendientes.count} consignación${cobrosBreakdown.consignacionesPendientes.count > 1 ? "es" : ""} pendiente${cobrosBreakdown.consignacionesPendientes.count > 1 ? "s" : ""}`,
                  context: `${fmtCOP(cobrosBreakdown.consignacionesPendientes.amount)} sin conciliar · bloquea flujo de caja`,
                  href: `/${orgSlug}/reconciliation`,
                  badgeColor: "#fbbf24",
                  badge: "ALTA PRIORIDAD",
                }] : []),
                ...((carteraKpis?.hasData && (carteraKpis.aging?.find(b => b.key === "90+")?.amount ?? 0) > 0) ? [{
                  label: "Gestionar cartera vencida +90 días",
                  context: `${fmtCOP(carteraKpis.aging?.find(b => b.key === "90+")?.amount ?? 0)} en riesgo · acción de cobro urgente`,
                  href: `/${orgSlug}/collections`,
                  badgeColor: "#f87171",
                  badge: "URGENTE",
                }] : []),
                ...(openTasks > 0 ? [{
                  label: `Revisar ${openTasks} tarea${openTasks > 1 ? "s" : ""} pendiente${openTasks > 1 ? "s" : ""}`,
                  context: "Acciones asignadas en progreso · requieren seguimiento",
                  href: `/${orgSlug}/agentik`,
                  badgeColor: "#60a5fa",
                  badge: "PENDIENTE",
                }] : []),
              ].slice(0, 4).map((item, idx) => (
                <a key={idx} href={item.href} style={{ textDecoration: "none", display: "block" }}>
                  <div style={{
                    display: "flex", alignItems: "center", gap: S[3],
                    background: "rgba(255,255,255,0.06)", borderRadius: R.md,
                    padding: `${S[2] + 2}px ${S[3]}px`,
                    border: "1px solid rgba(255,255,255,0.10)",
                  }}>
                    <span style={{
                      fontFamily: T.mono, fontSize: 9, fontWeight: T.wt.bold,
                      color: "#0f172a", background: item.badgeColor,
                      padding: "2px 6px", borderRadius: 3, flexShrink: 0,
                    }}>
                      {item.badge}
                    </span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: T.mono, fontSize: T.sz.sm, fontWeight: T.wt.bold, color: "#dbeafe" }}>
                        {item.label}
                      </div>
                      <div style={{ fontFamily: T.mono, fontSize: T.sz.xs, color: "#93c5fd" }}>
                        {item.context}
                      </div>
                    </div>
                    <span style={{ fontFamily: T.mono, fontSize: T.sz.sm, color: "#60a5fa", flexShrink: 0 }}>
                      → ejecutar
                    </span>
                  </div>
                </a>
              ...
              {criticalAlerts.length === 0 &&
               ...
               !(carteraKpis?.hasData && (carteraKpis.aging?.find(b => b.key === "90+")?.amount ?? 0) > 0) &&
               openTasks === 0 && (
                <div style={{ fontFamily: T.mono, fontSize: T.sz.sm, color: "#6ee7b7", padding: `${S[2]}px 0` }}>
                  Sin acciones urgentes · Operacion bajo control
                </div>
              )}
            </div>
          </div>
          ...
          {/* ══ CLOSING — EXECUTIVE SUMMARY ══════════════════════════════════════
               Agentik cierra la sesión ejecutiva con síntesis de estado + acción.
               ag-copilot-zone marks this as the AI synthesis layer — visually
               distinct from the operational blocks above via dashed separator.
               No filtros. No botones repetidos. Solo claridad.
               ════════════════════════════════════════════════════════════════ */}
          <div className="ag-copilot-zone">
          {(() => {
            // ── Derive closing state from real data ──
            const overdue90   = carteraKpis?.aging?.find(b => b.key === "90+")?.amount ?? 0;
            const consig      = cobrosBreakdown?.consignacionesPendientes;
            const hasOverdue  = carteraKpis?.hasData && (carteraKpis.overdueReceivable ?? 0) > 0;
            // TC-03: collectionRate from commercialKpis discarded (paidAmount=0 for SAG AR).
            // Warning signal uses cartera/overdue data only — reliable sources.

            // Salud: critical > warning > ok
            ...
            const healthLabel  = isCritical ? "Atención crítica requerida"
                               : isWarning  ? "Atención requerida"
                               :              "Operación estable";
            const healthColor  = isCritical ? "#dc2626"
                               : isWarning  ? "#b45309"
                               :              "#16a34a";
            const healthDot    = isCritical ? "#f87171"
                               : isWarning  ? "#fbbf24"
                               :              "#4ade80";

            // Principal riesgo
            const riesgo = overdue90 > 0
              ? `Cartera vencida +90 días · ${fmtCOP(overdue90)} en riesgo de incobrabilidad`
              : criticalAlerts.length > 0
              ? `${criticalAlerts.length} alerta${criticalAlerts.length > 1 ? "s" : ""} crítica${criticalAlerts.length > 1 ? "s" : ""} pendiente${criticalAlerts.length > 1 ? "s" : ""} de revisión ejecutiva`
              : (consig?.count ?? 0) > 0
              ? `${consig!.count} consignación${consig!.count > 1 ? "es" : ""} sin identificar · ${fmtCOP(consig!.amount)} bloqueando flujo de caja`
              : hasOverdue
              ? "Cartera vencida activa · revisar política de cobro"
              : "Sin riesgos críticos activos detectados";

            // Próxima acción prioritaria
            const accion = (consig?.count ?? 0) > 0
              ? "Identificar consignaciones pendientes · desbloquear flujo de caja"
              : overdue90 > 0
              ? "Comité de cobranza · cartera crítica +90 días"
              : criticalAlerts.length > 0
              ? "Revisar alertas ejecutivas · decisión requerida hoy"
              : "Seguimiento regular de indicadores · sin urgencias activas";

            // Recomendación síntesis
            const recomendacion = isCritical
              ? "Priorizar gestión de cartera crítica y revisión de alertas antes del cierre de semana."
              : isWarning
              ? "Atender consignaciones pendientes y monitorear evolución de cobros del período."
              : "Empresa en operación normal. Mantener cadencia de seguimiento semanal.";

            // Evolución operativa — temporal movement signal for the AI copilot layer
            // Derived from existing data. No new queries. Prepared for future agent enrichment.
            const evolucion = overdue90 > 0 && (carteraKpis?.overdueRatio ?? 0) > 30
              ? `Deterioro activo · cartera vencida representa ${(carteraKpis?.overdueRatio ?? 0).toFixed(0)}% del total · riesgo acumulando`
              : overdue90 > 0
              ? `Cartera +90 días activa · ${fmtCOP(overdue90)} en seguimiento intensivo · flujo de cobro en revisión`
              : (consig?.count ?? 0) > 0
              ? `Flujo bajo presión · ${consig!.count} consignación${consig!.count > 1 ? "es" : ""} sin identificar · liquidez bloqueada`
              : isWarning
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ width: 7, height: 7, borderRadius: "50%", background: healthDot }} />
                      <span style={{
                        fontFamily:    T.mono,
                        fontSize:      T.sz["2xs"],
                        fontWeight:    T.wt.bold,
                  ...
                  {/* ── Editorial blocks — executive synthesis ──
                       4 blocks: Riesgo · Acción · Recomendación · Evolución
                       The 4th block (EVOLUCIÓN OPERATIVA) is the temporal intelligence
                       layer — movement-aware, prepared for future agent enrichment. */}
                  {[
                    {
                    ...
                    {
                      eyebrow: "RECOMENDACIÓN AGENTIK",
                      value:   recomendacion,
                      urgent:  false,
                    },
                    {
                      eyebrow: "EVOLUCIÓN OPERATIVA",
                      value:   evolucion,
                      urgent:  false,
                    },
                  ].map((block, i, arr) => (
                    <div key={i} style={{
                      ...
                      <div style={{
                        ...
                        fontSize:      T.sz["2xs"],
                        fontWeight:    T.wt.bold,
                        color:         block.urgent ? "#dc2626" : C.inkFaint,
                        textTransform: "uppercase" as const,
                        letterSpacing: "0.07em",
                        marginBottom:  S[1] + 2,
                      }}>
                      ...
                      <div style={{
                        fontFamily:  T.mono,
                        fontSize:    T.sz.md ?? T.sz.sm,
                        fontWeight:  block.urgent ? T.wt.bold : T.wt.semibold,
                        color:       block.urgent ? "#dc2626" : C.ink,
                        lineHeight:  1.6,
                      }}>
                        {block.value}
                      </div>
                  ...
                  <div style={{
                    ...
                    <Link href={`/${orgSlug}/alerts`} style={{
                      ...
                      fontSize:       T.sz.xs,
                      fontWeight:     T.wt.bold,
                      color:          criticalAlerts.length > 0 ? "#dc2626" : C.inkMid,
                      textDecoration: "none",
                    }}>
                      Centro de decisiones →
                    </Link>
                    <span style={{ marginLeft: "auto", fontFamily: T.mono, fontSize: T.sz["2xs"], color: C.inkGhost }}>
                      Torre de Control · Agentik Executive
                    </span>
                  </div>
...
function TesoreriaOperativa({ orgSlug, apKpis, oldestAp }: { orgSlug: string; apKpis: ApKpis | null; oldestAp: ApDocumentRecord | null }) {
  ...
  return (
    <div style={{ marginBottom: S[4] }}>
      ...
      {/* Agentik tesorería strip */}
      <div className="ag-ai-strip" style={{
        padding: `${S[3]}px ${S[4]}px`,
        display: "flex", alignItems: "center", gap: S[3], marginTop: S[3],
      }}>
        <div style={{
          fontFamily: T.mono, fontSize: 9, fontWeight: T.wt.black,
          color: "#93c5fd", background: "rgba(59,130,246,0.20)",
          padding: "3px 8px", borderRadius: R.sm, letterSpacing: "0.08em", flexShrink: 0,
        }}>
          AGENTIK IA
        </div>
        <div style={{ fontFamily: T.mono, fontSize: T.sz.xs, color: "#dbeafe", flex: 1, lineHeight: 1.4 }}>
          <span style={{ color: "#93c5fd", fontWeight: T.wt.bold }}>Tesorería inteligente:</span>
          {" "}Registra tus obligaciones para que Agentik calcule cuánto apartar esta semana
          y te alerte antes de cada vencimiento crítico.
        </div>
      </div>
    </div>
  );
