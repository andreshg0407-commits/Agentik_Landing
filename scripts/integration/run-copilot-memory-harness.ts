/**
 * scripts/integration/run-copilot-memory-harness.ts
 *
 * HTTP client for the Copilot Memory Engine integration test harness.
 * Run: npx tsx scripts/integration/run-copilot-memory-harness.ts
 *
...
const MEMORY_BASE_URL = process.env.BASE_URL ?? "http://localhost:3000";
const MEMORY_ENDPOINT = `${MEMORY_BASE_URL}/api/internal/integration-tests/copilot-memory`;

async function main(): Promise<void> {
  console.log(`AGENTIK-COPILOT-MEMORY-ENGINE-01 — Integration Harness`);
  console.log(`Endpoint: ${MEMORY_ENDPOINT}\n`);

  const token = process.env.INTERNAL_INTEGRATION_TEST_TOKEN ?? "dev-integration-token";

  let res: Response;
  try {
    res = await fetch(MEMORY_ENDPOINT, {
      method:  "GET",
